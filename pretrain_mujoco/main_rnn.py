...
from pretrain_mujoco.utils import (BasicLogger, RecurrentActorProb,
                                   RecurrentCritic)
...
def main(args=get_args()):
    ...
    # collector
    if args.training_num > 1:
        buffers = [VectorReplayBuffer(
            args.buffer_size, len(train_envs), stack_num=args.stack_num) for _ in range(env.n_agents)]
    else:
        buffers = [ReplayBuffer(args.buffer_size, stack_num=args.stack_num) for _ in range(env.num_agents)]
